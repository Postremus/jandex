...
[TIP]
====
If you only need to know if an index contains an annotation, use `index.containsAnnotation(deprecated)`.

This is cheaper than  `!index.getAnnotations(deprecated).isEmpty()`, since less allocations are needed for composite indexes.
====
